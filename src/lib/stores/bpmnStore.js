...
const createBpmnStore = () => {
    ...
    return {
                                ...
                                if (isPool) {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        if (el.waypoints) {
                                            ...
                                            update(elements => elements.map(element => element.id === el.id ? { ...element, waypoints: refreshedWaypoints } : element));
                                        }
                                    }, 0);
                                }
                            }
        ...
        updateConnectionWaypoints: (id, waypoints) => {
            ...
            return update(elements => elements.map(el => {
                if (el.id === id && el.type === 'connection') {
                    return { ...el, waypoints: waypoints };
                }
                return el;
            }));
        },
            ...
            return elements.map(el => {
                if (el.id === id && el.type === 'connection') {
                    ...
                    if (sourcePointId !== undefined)
                        updatedConnection.sourcePointId = sourcePointId;
                    if (targetPointId !== undefined)
                        updatedConnection.targetPointId = targetPointId;
        ...
        updateConnectionLabelOffset: (id, offset) => update(elements => elements.map(el => el.id === id && el.type === 'connection' ? { ...el, labelOffset: offset } : el)),
