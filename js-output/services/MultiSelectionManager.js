...
export class MultiSelectionManager {
    constructor() {
        ...
        this.selectedElementIds = writable([]);
        // Store for original positions when dragging multiple elements
        this.originalPositions = writable({});
        // Selection mode flag
        this.selectionMode = writable(false);
        // Selection rectangle coordinates
        this.selectionRect = writable({
            startX: 0,
            startY: 0,
            endX: 0,
            endY: 0,
    ...
    endSelectionRect() {
        ...
        const containingPoolsAndLanes = elements.filter(element => {
            ...
            return (normalizedRect.left >= element.x &&
                normalizedRect.right <= element.x + element.width &&
                normalizedRect.top >= element.y &&
                normalizedRect.bottom <= element.y + element.height);
        });
        console.log('DEBUG: Selection rectangle is inside these pools/lanes:', containingPoolsAndLanes.map(el => el.id));
        ...
        const elementsInRect = elements.filter(element => {
            ...
            return (element.x < normalizedRect.right &&
     ...
     */
    selectElements(elementIds, addToSelection = false) {
