 ...
 */
export function calculateConnectionPoints(element) {
    ...
    if (element.type === 'task' || element.type === 'event' || element.type === 'gateway' ||
        ...
        points.push(createConnectionPoint(element.id, 'top', centerX, element.y), createConnectionPoint(element.id, 'right', element.x + element.width, centerY), createConnectionPoint(element.id, 'bottom', centerX, element.y + element.height), createConnectionPoint(element.id, 'left', element.x, centerY));
    }
    else if (element.type === 'pool') {
        // Pools have connection points on all four sides, but with more points for better connectivity
        // Top edge
        points.push(createConnectionPoint(element.id, 'top', element.x + element.width * 0.25, element.y), createConnectionPoint(element.id, 'top', centerX, element.y), createConnectionPoint(element.id, 'top', element.x + element.width * 0.75, element.y));
        // Right edge
        points.push(createConnectionPoint(element.id, 'right', element.x + element.width, element.y + element.height * 0.25), createConnectionPoint(element.id, 'right', element.x + element.width, centerY), createConnectionPoint(element.id, 'right', element.x + element.width, element.y + element.height * 0.75));
        // Bottom edge
        points.push(createConnectionPoint(element.id, 'bottom', element.x + element.width * 0.25, element.y + element.height), createConnectionPoint(element.id, 'bottom', centerX, element.y + element.height), createConnectionPoint(element.id, 'bottom', element.x + element.width * 0.75, element.y + element.height));
        // Left edge
        points.push(createConnectionPoint(element.id, 'left', element.x, element.y + element.height * 0.25), createConnectionPoint(element.id, 'left', element.x, centerY), createConnectionPoint(element.id, 'left', element.x, element.y + element.height * 0.75));
    }
 ...
 */
export function createConnectionPoint(elementId, position, x, y) {
    return {
 ...
 */
export function calculateConnectionPath(start, end, waypoints, routingType = 'orthogonal') {
    if (waypoints && waypoints.length > 0) {
        // Use waypoints if provided
        let path = `M ${start.x} ${start.y}`;
        // Check if the first waypoint is close to the start point
        const firstWaypoint = waypoints[0];
        const distToFirstWaypoint = Math.sqrt(Math.pow(start.x - firstWaypoint.x, 2) +
        ...
        const distToLastWaypoint = Math.sqrt(Math.pow(end.x - lastWaypoint.x, 2) +
 ...
 */
function getAnchorDirection(p1, p2) {
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
    const adx = Math.abs(dx);
    const ady = Math.abs(dy);
 ...
 */
export function isValidConnection(_sourceType, _targetType) {
 ...
 */
function calculateOrthogonalSegment(start, end) {
    // Corner radius for rounded corners
    const cornerRadius = 8;
 ...
 */
export function generateVirtualBendpoints(start, end, waypoints, segmentThreshold = 50) {
    const virtualPoints = [];
 ...
 */
export function updateWaypoint(waypoints, index, newX, newY, snapToGrid = true, gridSize = 20) {
 ...
 */
export function addWaypoint(waypoints, newX, newY, snapToGrid = true, gridSize = 20) {
    // Snap the coordinates to the grid if needed
    const x = snapToGrid ? gridSnap(newX, gridSize) : newX;
    const y = snapToGrid ? gridSnap(newY, gridSize) : newY;
 ...
 */
export function calculateLabelPosition(start, end, waypoints) {
    if (waypoints && waypoints.length > 0) {
